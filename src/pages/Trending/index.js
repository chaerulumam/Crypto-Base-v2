...
import { TrendingCoinDetails } from "../../components";
import { TrendingContext } from "../../context/TrendingContext";

const Trending = () => {
  let { trendingData, resetTrendingResult } = useContext(TrendingContext);
      ...
      <div className="w-full min-h-[60vh] py-8 flex flex-wrap justify-evenly border border-gray-100 rounded">
        {trendingData &&
          trendingData.map((coin) => (
            <TrendingCoinDetails key={coin.coin_id} data={coin.item} />
          ))}
        <button
