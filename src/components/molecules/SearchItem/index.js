...
const SearchInput = ({ handleSearchData }) => {
  ...
  return (
    <>
      ...
      {searchText.length > 0 ? (
        <>
          <ul className="absolute top-11 right-0 w-96 h-96 rounded overflow-x-hidden py-2 bg-gray-200 bg-opacity-60 backdrop:blur-md scrollbar-thin scrollbar-thumb-amber-500 scrollbar-track-gray-100">
            {searchData ? (
              searchData.map((coin) => {
                return (
                  <>
